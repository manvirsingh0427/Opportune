...
const JobListing = () => {
    ...
    const handleLocationChange = (loc)=> {
         setSelectedLocations(
            prev => prev.includes(loc) ? prev.filter(l => l !== loc) : [...prev, loc]
         )
    }
  ...
  return (
    <div className='container 2xl:px-20 mx-auto flex flex-col lg:flex-row max-lg:space-y-8 py-8'>
                  ...
                  <div className='mb-4 text-gray-600'>
                    {searchFilter.title && (
                        <span className='inline-flex items-center gap-2.5 bg-blue-50 border border-blue-200 px-4 py-1.5 rounded'>
                          {searchFilter.title}
                          <img src={assets.cross_icon} alt="" className='cursor-pointer' onClick={removeSearch}/>
                        </span>
                    )} 
                    {searchFilter.location && (
                        <span className='ml-2 inline-flex items-center gap-2.5 bg-red-50 border border-red-200 px-4 py-1.5 rounded'>
                            {searchFilter.location}
                            <img src={assets.cross_icon} alt="" className='cursor-pointer' onClick={removeLoc} />
                        </span>
                    )}
                  </div>
                </>
                ...
                <ul className='space-y-4 text-gray-600'>
                    {
                        JobCategories.map((category, index)=>(
                            <li className='flex gap-3 items-center' key={index}>
                                ...
                                className='scale-125' 
                                type="checkbox" 
                                onChange={()=>handleCatergoryChange(category)}
        ...
        <section className='w-full lg-w-3/4 text-gray-800 max-lg:px-4'>
            ...
            <p className='mb-8'>Get your job from top companies</p>
